...
def create_networksLSTM(tr_x, tr_y, layers_arr):
    ...
    for idx in range(len(layers_arr)):
        ...
        model.add(tf.keras.layers.LSTM(int(layers_arr[idx, 0]), input_shape=(tr_x.shape[1], tr_x.shape[2]),
                                       return_sequences=True))   #  input_shape=(tr_x.shape[1], tr_x.shape[2]),
        ...
        model.fit(tr_x, tr_y, epochs=50, batch_size=32, # epochs=50, batch_size=32
...
def plot_mae(val_x, mae, angle_range, LSTM):
...
def print_average_mae(mae_angle_range, LSTM):
    pass
...
for LSTM in range(0, 2):
    for angle_range in range(0, 3):
        average_mae = plot_mae(val_x, mae, angle_range, LSTM)
...
print_average_mae(mae_angle_range[LSTM, angle_range], LSTM)
"""
...
# plot_mae(mae[0])  # to plot mae of first model for all steps
